...
# decorative. Postgres only: no payments provider is contacted. A negotiated
# deal's MONEY does live here (--flat-fee and --rate-per-million, migration 021):
# pug prices the period, the provider collects it, so a deal with no terms
# recorded is a deal nothing can invoice. Ungated by PUG_BILLING_ENABLED,
...
# the interesting bugs live in the gap: a lapsed deal's terms and quota are
# invisible in the resolved answer and still carry onto the next `set`.
...
# its empty value (--events 0, --flat-fee 0, --rate-per-million 0,
# --retention-days 0, --name "", --anchor-day 0, --until "") clears it.
# --plan custom is REFUSED without a fee or a rate (ErrCustomNeedsPrice, mirrored
# by the custom_needs_price CHECK): a deal that charges nothing is not a deal. An
# allowance is optional — a rate with no --events charges from the first event.
...
- **Billing entitlement** — what an org is *allowed* to send, beside metering's count of what it did. A Go catalog of graduated **rate cards** (not rows — repricing mints a new slug and retires the old, so a holder keeps the numbers they bought), one nullable `billing_entitlements` row per org (migration 019; the deal's money columns are 021), negotiated deals as per-org **terms** (flat fee, cents per million) plus quota/retention/display-name overrides, an append-only history, and every status **derived at read time** from the clock, so there is no stored status and no sweep job. Read-only over the wire: `GetBillingStatus` sits on the viewer floor (`authz.ResourceBilling`), and `included_events`/`retention_days` are `Int64Value` wrappers because **absent means no quota / no retention bound and must never render as 0**. There is **no list price**: a graduated card is a table, so `Plan.price_cents` is `reserved 3` on the wire and the response carries `rate_card` (the card's tiers) or `custom_terms` (a deal's fee/rate/allowance) instead — exactly one, or neither when nothing prices the org. **Retention is a plan term nothing enforces**: a card's `RetentionDays` (`CardRetentionDays`, 5 years; per-org override for a deal) is rendered, never pruned on — there is no TTL, delete job or query clamp anywhere, so pug keeps everything. Enforcement is a structural invariant *against*: no ingestion path imports any billing package and no event is ever rejected, throttled or dropped for a quota — a quota drives a banner. `PUG_BILLING_ENABLED` off (the default, and the self-hosted shape) resolves every org with no quota at all. The operator CLI is `pug billing` (`show`/`set`/`extend-trial`/`clear`) — Postgres only, ungated by the switch so a deployment can be prepared before it goes on, and the **only** writer of `billing_entitlements`; an omitted override flag keeps the stored value and its empty value clears it, since the common re-`set` is a renewal; `--plan custom` is refused without a fee or a rate, while an allowance is optional (a rate with no `--events` charges from the first event) → [`docs/architecture/billing.md`](docs/architecture/billing.md)

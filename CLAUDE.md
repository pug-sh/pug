...
# decorative. Postgres only: no payments provider is contacted, but a deal's
# money lives HERE now: --flat-fee and --rate-per-million are the terms pug
# prices and charges on. Ungated by PUG_BILLING_ENABLED,
...
# `set` pins a rate card or records a deal, merging the flags given over whatever
# is stored: an omitted flag KEEPS the stored value (the common re-set is a
# renewal on unchanged terms) and its empty value (--flat-fee 0,
# --rate-per-million 0, --events 0, --retention-days 0, --name "",
# --anchor-day 0, --until "") clears it. --plan custom needs a flat fee or a rate
# — an allowance alone is a free tier nobody agreed to — and --events is its
# allowance; on any other plan the money is cleared whether or not the flag was
# passed. --plan "" removes the pin and keeps the rest of the row.
# --until is INCLUSIVE of the day given; the stored instant is the following
# midnight, and `show` prints both. Every write is attributed to --actor and
# appended to the org's history in the same transaction.
./bin/pug billing set o_2f9k --plan custom --flat-fee 40000 --rate-per-million 3000 \
                             --events 5000000 --retention-days 2555 \
                             --name "Acme Enterprise" \
                             --until 2027-01-01 --note '$400/mo, INV-123' \
                             --actor "praveen/INV-123"

# extend-trial sets an absolute now + --days, so it can only lengthen a trial; a
# deal in force blocks it, a card pin does not. clear deletes the row, returning
# the org to the current card — a live mandate goes on being charged, now at
# list rates, so clearing a deal is a price change rather than a refusal.
# preview prices --events on the org's own deal, or on the card its mandate
# pinned, through the same two functions an invoice uses. Read-only.
./bin/pug billing extend-trial <org-id> --days 30 --actor <who>
./bin/pug billing clear <org-id> --actor <who>
./bin/pug billing preview <org-id> --events 5000000
```
...
- **Billing entitlement** — what an org is *allowed* to send, beside metering's count of what it did. A Go rate-card catalog (not rows — repricing mints a new slug and retires the old), one nullable `billing_entitlements` row per org (migrations 019 + 021), negotiated deals as per-org money/quota/retention/display-name overrides, an append-only history, and every status **derived at read time** from the clock, so there is no stored status and no sweep job. Read-only over the wire: `GetBillingStatus` sits on the viewer floor (`authz.ResourceBilling`), and `included_events`/`retention_days` plus every `CustomTerms` field are `Int64Value` wrappers because **absent means no quota / no retention bound / no such term and must never render as 0**; `price_cents` is `reserved` — usage is priced per event, so `rate_card` or `custom_terms` carries the price, exactly one of the two. **Retention is a plan term nothing enforces**: a card's `RetentionDays` (5 years on every card; per-org override for a deal) is rendered, never pruned on — there is no TTL, delete job or query clamp anywhere, so pug keeps everything. Enforcement is a structural invariant *against*: no ingestion path imports the package and no event is ever rejected, throttled or dropped for a quota — a quota drives a banner. `PUG_BILLING_ENABLED` off (the default, and the self-hosted shape) resolves every org with no quota at all. `free` and `trial` are derived STATES, not plans: an empty `plan_slug` is what "no pin" looks like. The operator CLI is `pug billing` (`show`/`set`/`extend-trial`/`clear`/`preview`) — Postgres only, ungated by the switch so a deployment can be prepared before it goes on, and the **only** writer of `billing_entitlements`; an omitted flag keeps the stored value and its empty value clears it, since the common re-`set` is a renewal → [`docs/architecture/billing.md`](docs/architecture/billing.md)
...
(a standalone binary, not a `pug cron` subcommand). **A returning buyer is confirmed by `ConfirmCheckout`, not by the webhook** — it re-reads the checkout straight from the provider and applies it through the same CAS, because polling pug's own state verifies nothing and makes the checkout moment depend on a reachable webhook URL, which a self-hosted or local deployment does not have. The `session_id` it takes is a claim: the subscription must carry the `metadata.org_id` pug wrote **and** a `checkout_ref` that resolves to the same org — a ref-less confirm is refused and stays the webhook's to place, and there is no customer-id fallback. The webhook stays the authority for the lifecycle (renewal, dunning, cancellation, expiry), which has no redirect to be carried on. **Pug owns the price as well as the quota; the provider only moves the money** — every org authorizes against the one `PUG_DODO_MANDATE_PRODUCT`, whose own recurring price is never charged, and a deal's money is written by an operator with `pug billing set --flat-fee --rate-per-million`. One writer per row: the operator writes `billing_entitlements`; the payments side writes `billing_subscriptions`, and its three writers (webhook, reconcile, `ConfirmCheckout`) all go through the one CAS in `applySubscription` so no second notion of "newer" exists, under the org lock that makes the card read and the write one step — two deliveries for one subscription would otherwise both miss the stored slug and the later one price the mandate on a card nobody pinned. A concurrent `billing clear` has nothing to strand, since the mandate resolves nothing from the org's row. A delivery is attributed on a `checkout_ref` pug mints and stores in `billing_checkout_sessions` **before** it opens the checkout, then on the provider customer while it names a single org — never on `metadata.org_id`, which a provider's static payment links let the buyer set from the URL, so a payload's org id names an org rather than proving one. Every unapplicable delivery is stored, marked processed and NOT retried (foreign currency, no status, no customer, unattributable, and — since every mandate is charged by pug rather than billed by the provider — a subscription that is not `on_demand` or that is tax-inclusive) — retries fix none of them. A delivery's **product decides nothing**, since every org buys the same one: the card comes from the checkout's pinned slug, else the subscription's stored slug, else the current card, so a renewal or cancellation carrying no ref cannot move a grandfathered price. Two deliveries ARE retried: a second *live* subscription for one org, which trips the partial unique index and whose ordinary cause is a cutover whose cancellation has not landed yet; and a body pug cannot **decode**, which is a payload shape that changed under it and which a redeploy inside the retry window fixes — consuming that one would lose the replay. The CAS tie-breaks in the withholding direction: a webhook's stamp is the whole-second `webhook-timestamp` header, so an equal stamp can end a subscription but never revive one. `past_due` is **live**: a failed card is worth a banner, never a degraded product. Enforcement stays out here too → [`docs/architecture/payments.md`](docs/architecture/payments.md)

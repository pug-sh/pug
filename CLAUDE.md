...
# Operator CLI for org domains. Postgres only; settings stay the org admins' choice,
# except `unenforce`. `verify` marks a domain verified without DNS, `show` lists
# every org that added it, and `release` drops one org's claim. `unenforce` turns
# Require SSO off in every org: the way back when SSO breaks and nobody on the
# domain can sign in.
...
# ever written here (what a negotiated deal is charged lives in the provider;
# --note is where an operator writes it down). Ungated by PUG_BILLING_ENABLED,
# so a deployment can be prepared before the switch goes on — the flag decides
# only how `show` reports the RESOLVED half.
# `show` prints the resolved entitlement AND the stored row beneath it, because
# the interesting bugs live in the gap: a lapsed deal's quota is invisible in the
# resolved answer and still carries onto the next `set`.
./bin/pug billing show <org-id> [--history]

# `set` merges the flags given over whatever is stored: an omitted override flag
# KEEPS the stored value (the common re-set is a renewal on unchanged terms) and
# its empty value (--events 0, --retention-days 0, --name "", --anchor-day 0,
# --until "") clears it.
# --until is INCLUSIVE of the day given; the stored instant is the following
# midnight, and `show` prints both. Setting a FLOOR plan (free/trial) with no
# --until clears the contract and the overrides it gated — a real date there is a
# comped grant and keeps them; on a paid plan an empty --until clears only the
# contract. Every write is attributed to --actor and appended to the org's
# history in the same transaction.
./bin/pug billing set o_2f9k --plan custom --events 5000000 --retention-days 2555 \
                             --name "Acme Enterprise" --provider-product prod_2f9k \
...
# Refused while a live CUSTOM subscription is resolving its quota from that row:
# deleting it would drop an org that is still being charged to the free floor.
...
- **Billing entitlement** — what an org is *allowed* to send, beside metering's count of what it did. A Go plan catalog (not rows — repricing mints a new slug and retires the old), one nullable `billing_entitlements` row per org (migration 019), negotiated deals as per-org quota/retention/display-name overrides, an append-only history, and every status **derived at read time** from the clock, so there is no stored status and no sweep job. Read-only over the wire: `GetBillingStatus` sits on the viewer floor (`authz.ResourceBilling`), and `included_events`/`retention_days`/`price_cents` are `Int64Value` wrappers because **absent means no quota / no retention bound / no list price and must never render as 0**. **Retention is a plan term nothing enforces**: a tier's `RetentionDays` (1 year on the floors and starter, 3 on growth, 7 on scale; per-org override for a deal) is rendered, never pruned on — there is no TTL, delete job or query clamp anywhere, so pug keeps everything. Enforcement is a structural invariant *against*: no ingestion path imports the package and no event is ever rejected, throttled or dropped for a quota — a quota drives a banner. `PUG_BILLING_ENABLED` off (the default, and the self-hosted shape) resolves every org with no quota at all. The operator CLI is `pug billing` (`show`/`set`/`extend-trial`/`clear`) — Postgres only, ungated by the switch so a deployment can be prepared before it goes on, and the **only** writer of `billing_entitlements`; an omitted override flag keeps the stored value and its empty value clears it, since the common re-`set` is a renewal → [`docs/architecture/billing.md`](docs/architecture/billing.md)
- **Payments** — taking money, beside the entitlement that says what an org may send. A merchant of record behind `billing.PaymentProvider` (**Dodo Payments** is the only implementation, `internal/deps/dodo`), self-serve checkout and a customer portal (both admin-only RPCs; plan changes and cancellation live in the provider's portal, so there is no `ChangePlan`), a signed Standard-Webhooks inbox at `/billing/webhooks/<provider>` mounted **outside** the Connect chain, and a `cmd/cron/billing-reconcile` pass
(a standalone binary, not a `pug cron` subcommand). **A returning buyer is confirmed by `ConfirmCheckout`, not by the webhook** — it re-reads the checkout straight from the provider and applies it through the same CAS, because polling pug's own state verifies nothing and makes the checkout moment depend on a reachable webhook URL, which a self-hosted or local deployment does not have. The `session_id` it takes is a claim: the subscription must carry the `metadata.org_id` pug wrote **and** a `checkout_ref` that resolves to the same org — a ref-less confirm is refused and stays the webhook's to place, and there is no customer-id fallback. The webhook stays the authority for the lifecycle (renewal, dunning, cancellation, expiry), which has no redirect to be carried on. **Pug owns the quota; the provider owns the money** — no price is ever written by a person, and `pug billing set --provider-product` pastes the one id a negotiated deal needs. One writer per row: the operator writes `billing_entitlements`; the payments side writes `billing_subscriptions`, and its three writers (webhook, reconcile, `ConfirmCheckout`) all go through the one CAS in `applySubscription` so no second notion of "newer" exists — under the same `pg_advisory_xact_lock` `billing clear` takes, re-reading the entitlement row inside it, or a clear between the product mapping and the write strands a live custom subscription on the free floor. A delivery is attributed on a `checkout_ref` pug mints and stores in `billing_checkout_sessions` **before** it opens the checkout, never on `metadata.org_id` alone — a provider's static payment links accept `metadata_*` query parameters, so a payload's org id names an org rather than proving one; it counts only paired with the `provider_product_id` an operator already staged for that org. Every unapplicable delivery is stored, marked processed and NOT retried (foreign currency, no status, no customer, negative price, unmappable product, unattributable) — retries fix none of them. The unmappable-product refusal is **granting-direction only**: a non-live status keeps the row's stored `plan_slug` instead of resolving one, because refusing a cancellation whose product left the config would leave the org on a tier it stopped paying for. Two deliveries ARE retried: a second *live* subscription for one org, which trips the partial unique index and whose ordinary cause is a cutover whose cancellation has not landed yet; and a body pug cannot **decode**, which is a payload shape that changed under it and which a redeploy inside the retry window fixes — consuming that one would lose the replay. The CAS tie-breaks in the withholding direction: a webhook's stamp is the whole-second `webhook-timestamp` header, so an equal stamp can end a subscription but never revive one. `past_due` is **live**: a failed card is worth a banner, never a degraded product. Enforcement stays out here too → [`docs/architecture/payments.md`](docs/architecture/payments.md)
- **SSO & verified domains** — phases 1 and 2 of 3. An org verifies a domain with a DNS TXT record (`_pug-verification.<domain>`) or through the operator (`pug domains verify`). Auto-join is off by default and never grants admin. It adds people whose SSO sign-in proves the domain, through Google's `hd` or a provider's `emailDomains`. Pug reads `hd` only when the provider's *configured* issuer is Google's. An email link never proves a domain. If any org that verified a domain turns off "Let members create their own orgs", nobody with an email on it can create an org, except admins of an org that verified it. Turning auto-join on or up to Member, or org creation off, re-checks the org's DNS records. Removal doesn't stick yet: while auto-join is on, a removed member is added back at their next SSO sign-in or session refresh. Auto-join also skips orgs with a pending, unexpired invite for the email. A refresh token keeps the domain its SSO sign-in proved (`proven_domain`), and `RefreshSession` re-runs auto-join after the refresh commits. A failure there is only logged. **Require SSO** (phase 2) is a per-claim switch (`UpdateDomain`); any org that verified the domain can turn it on, so the strictest wins. Turning it on needs a verified claim with `sso_seen_at` set and re-checks its TXT record. `coreorgs.CheckSignInInTx` then refuses, with `SSO_REQUIRED`, every password sign-in, email link (left unused), `SetPassword`, and provider sign-in that doesn't prove the email's domain, checking both the token's email and the account its `sub` resolves to. `RefreshSession` refuses the same way and revokes the token. It returns `Unauthenticated`, because the frontend ends a session only on that code; the demo viewer is exempt. The error's `public.auth.v1.SSORequired` detail names the providers that can sign the domain in and whether the refused link was an invite. The frontend then sends that link's token as `CompleteOIDCSignIn.invite_token` → [`docs/architecture/sso.md`](docs/architecture/sso.md), operator config → [`docs/authentication.md`](docs/authentication.md)

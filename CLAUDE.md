...
- **`Upsert` is the only RPC that directly mutates tiles.** (Deleting the parent dashboard cascades to its tiles via the FK.) There are no per-tile RPCs — `CreateTile`/`UpdateTile`/`DeleteTile` were deliberately removed so every tile write reconciles against the full draft. The handler runs in a single transaction: tiles with empty `id` are inserted; tiles with a matching `id` are updated; existing tiles whose `id` is omitted from the request are deleted; duplicate non-empty ids in one request are rejected (`ErrDuplicateUpsertTileID` → `CodeInvalidArgument`). UPDATE statements are gated in SQL on `payload_hash <> $new_hash`, so byte-identical writes don't fire the `moddatetime` trigger and `update_time` stays meaningful. The dashboard metadata UPDATE is gated symmetrically on `tiles_changed OR metadata changed`. The response returns tiles in **request order** so clients can map server-assigned ids back to a local draft. The contract is last-write-wins — no optimistic locking; concurrent edits aren't detected server-side.
- **`payload_hash`:** sha256 over `proto.MarshalOptions{Deterministic: true}` of a `DashboardTileInput` built from the **normalized** tile fields, with `id` left zero. Normalization runs in `computeTilePayloadHash` and is what keeps the hash equal to a re-Upsert of what Get returns: view_mode and compare go through their normalizers, layouts are sorted by breakpoint (storage is a keyed map — read returns alphabetical order), and zero-valued `Header`/`Visualization` collapse to nil (storage stores them as `{}` either way). Written on every insert/update so the short-circuit predicate stays correct across rewrites.

...
# Operator CLI for org domains. Postgres only; it never changes an org's settings.
# `verify` marks a domain verified without DNS, `show` lists every org that added
# it, and `release` drops one org's claim.
...
- **SSO & verified domains** — phase 1 of 3. An org verifies a domain with a DNS TXT record (`_pug-verification.<domain>`) or through the operator (`pug domains verify`). Auto-join is off by default and never grants admin. It adds people whose SSO sign-in proves the domain, through Google's `hd` or a provider's `emailDomains`. Pug reads `hd` only when the provider's *configured* issuer is Google's. An email link never proves a domain. If any org that verified a domain turns off "Let members create their own orgs", nobody with an email on it can create an org, except admins of an org that verified it. Turning auto-join on or up to Member, or org creation off, re-checks the org's DNS records. Removal doesn't stick yet: while auto-join is on, a removed member is added back at their next SSO sign-in or session refresh. Auto-join also skips orgs with a pending, unexpired invite for the email. A refresh token keeps the domain its SSO sign-in proved (`proven_domain`), and `RefreshSession` re-runs auto-join after the refresh commits. A failure there is only logged → [`docs/architecture/sso.md`](docs/architecture/sso.md), operator config → [`docs/authentication.md`](docs/authentication.md)

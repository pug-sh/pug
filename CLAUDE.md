...
- **bot_score enrichment** — reads the `CF-Bot-Score` header (injected via Cloudflare Transform Rule) and sets `$bot_score` in `event.AutoProperties`. Like geo, the value is **always overwritten** by the server; client-supplied `$bot_score` is stripped before enrichment. Stored as a string in auto-properties (e.g. `"42"`), the score ranges from 0–255 where lower values indicate more bot-like traffic.

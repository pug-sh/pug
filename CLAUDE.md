...
# decorative. Postgres only: no payments provider is contacted, and a deal's
# money IS written here now (pug computes every amount; --note stays the
# paperwork). Ungated by PUG_BILLING_ENABLED, so a deployment can be prepared
# before the switch goes on — the flag decides only how `show` reports the
# RESOLVED half.
# `show` prints the resolved entitlement AND the stored row beneath it, because
# the interesting bugs live in the gap: a lapsed deal's terms are invisible in
# the resolved answer and still carry onto the next `set`. --invoices adds the
# ledger, newest first, which is the operator's view of a dunning conversation.
./bin/pug billing show <org-id> [--history] [--invoices]

# `set` merges the flags given over whatever is stored: an omitted flag KEEPS the
# stored value (the common re-set is a renewal on unchanged terms) and its empty
# value (--flat-fee 0, --block-rate 0, --events 0, --retention-days 0, --name "",
# --anchor-day 0, --until "") clears it. --plan takes a rate-card slug, `custom`
# or `free`; a custom deal needs a --flat-fee or a --block-rate (an allowance
# alone is a free tier nobody agreed to), and --events is its allowance, in whole
# 100,000-event blocks.
# Changing either money flag stamps terms_effective_at, which is the day the deal
# is invoiced from: the row usually predates the deal, so its create_time would
# retroactively bill every elapsed period at the new rate.
# --until is INCLUSIVE of the day given; the stored instant is the following
# midnight, and `show` prints both. Setting `free` with no --until clears the
# contract and the terms it gated — a real date there is a comped grant and keeps
# them. Every write is attributed to --actor and appended to the org's history in
# the same transaction.
./bin/pug billing set o_2f9k --plan custom --flat-fee 40000 --block-rate 300 \
                             --events 5000000 --retention-days 2555 \
                             --name "Acme Enterprise" \
                             --until 2027-01-01 --note '$400/mo, INV-123' \
                             --actor "praveen/INV-123"

# preview is Price() on the org's own card or terms — what the customer will be
# charged, which is how a deal is sanity-checked before it is set.
./bin/pug billing preview <org-id> --events 2340000

# extend-trial sets an absolute now + --days, so it can only lengthen a trial;
# clear deletes the row, returning the org to the derived trial-then-free floors
# and the current rate card.
./bin/pug billing extend-trial <org-id> --days 30 --actor <who>
./bin/pug billing clear <org-id> --actor <who>

# void is the only way to stop a charge before it happens; it never calls the
# provider and it cannot touch a paid invoice, whose only onward state is
# refunded and whose only writer is the refund webhook.
# retry is for the customer who fixed their card by phone.
./bin/pug billing invoice void <invoice-id> --actor <who> --note "..."
./bin/pug billing invoice retry <invoice-id> --actor <who>
```
...
- New accounts are created by completing a magic link or OAuth sign-in — there is no password-signup endpoint (`SignUpWithEmail` was removed). Completing a *plain* (non-invite) magic link for a new email provisions a default org + project atomically (in `CompleteMagicLink`); an *invite* magic link instead joins the inviting org with the invitation's role. `CompleteMagicLink` looks the token up by hash (unique) and dispatches on its `purpose`: it honors only `"magic_link"` and `"org_invite"`, rejecting any other purpose with `ErrInvalidToken` so a token minted for a future flow can't be redeemed as a login. `CompleteOIDCSignIn` accepts a dynamically configured provider ID plus the browser's authorization code, PKCE verifier, redirect URI, and nonce; Pug exchanges the code and verifies the ID token server-side, then resolves the IdP identity against `customer_identities` (link by verified email or create) and runs the same shared provisioning path as magic link (`FinishSignup` + `FinalizeVerifiedCustomer` in `provision.go`) in a single transaction (with fresh-transaction retries on signup races) via `oauth.WithIdentityTx`. Provider client secrets stay in `PUG_CONFIG_FILE` and are never returned by `GetAuthConfig` — the public, unauthenticated RPC the browser reads to render sign-in buttons and start discovery (id, type, display name, client id, issuer, scopes). Every external provider, including Google, uses the generic OIDC flow, and the identity is stored as the configured provider **`id`** + the ID token's `sub` — each provider is one issuer and client, so `sub` is unique within it. That makes `id` a permanent identity key, not a renameable label: renaming it orphans existing links, which then re-resolve through the verified-email fallback and leave a stale row. `NewVerifiedIdentity` rejects an empty provider and `ExchangeCode` rejects an identity whose provider doesn't match the one asked, so the namespace can't be filed wrong. The ID token's `nonce` is checked against the one the browser sends with the code, and an empty expected nonce fails closed (proto `required` is only a presence check under editions, so `min_len` on the field is what stops `""`). The server neither mints nor stores the nonce, so this is a consistency check between the token and the request, not a server-enforced session binding — PKCE and the IdP's redirect-URI registration are what actually bind the code to the browser that started the flow. A verified IdP email links to an existing email/password or magic-link account without clearing `password_hash`. Password login (`SignInWithEmail`) and authenticated `SetPassword` remain, so a magic-link account can opt into a password and then sign in with it.
...
- **Tile kinds:** `insight` (persisted `insight_query` JSONB holding a `shared.insights.v1.InsightQuerySpec` + `view_mode` column) or `markdown` (`markdown_body`). Insight `view_mode` includes line/area/bar/pie/table/KPI/**Sankey** (`DASHBOARD_TILE_VIEW_MODE_SANKEY`)/**Map** (`DASHBOARD_TILE_VIEW_MODE_MAP`) — a client rendering hint only; the server stores and echoes it without enforcing insight↔view_mode pairing. Sankey and Map each belong to exactly one insight type (USER_FLOW, MAP) and are the only view that type takes; the FE pins the pairing, the server does not. Adding a view mode means adding it to **both** exhaustive switches — `normalizedTileViewModeProto` (write) and `TileViewModeToRPC` (read) — or the tile silently degrades to LINE. The time window and granularity are **dashboard-level**, not per-tile — a tile only stores *what* it measures.
...
**Request size:** `rpc.MaxRequestBytes` (8 MiB) is the only bound on total request size — the per-field proto limits compose to far more (a proto-valid 1000-event batch can exceed 50 MiB, since the property maps have no `max_pairs`), and Connect buffers the whole message to unmarshal it. It is enforced twice: `rpc.WithRequestLimits` wraps the entire mux (so `/mcp` and any future non-Connect route are covered) and caps the bytes on the wire, while `connect.WithReadMaxBytes` in `handlerOpts` caps the **decompressed** message — without it a small gzipped body still unmarshals unbounded. Both surface as `CodeResourceExhausted`. The same middleware gives the body its own upload deadline (`bodyReadTimeout`, 2m); `ReadTimeout` stays unset on the `http.Server` because it would cap the body as tightly as the headers.
...
- **Insights** — trends/funnel/retention/segmentation/**user flow (Sankey)**/**top K (ranked dimension + optional $others bucket)**/**map (a metric per country, choropleth)** queries; breakdowns, granularity caps, filter model, funnel timing stats, type-specific query builders; **`include_cookieless` spec toggle** (default false: user-counting metrics UNIQUE_USERS/PER_USER_AVG and person-based insights funnel/retention/user-flow/USER-top-K exclude `cookieless-` ids; TOTAL, numeric aggregations (SUM/AVG/MIN/MAX) and all session metrics always count all traffic; migration 011's rollup `cookieless` key column keeps both toggle states on the fast path); **`include_bots` spec toggle** (default false: every insight and every metric excludes ingest-tagged bot events — row-level `bot = 0` on events and the event rollup, session-level `HAVING max(bot) = 0` (`chq.SessionBotHaving`) on the raw session CTE, the session rollup, user flow and `shared.activity`'s `GetProfileSessions` so a straddling session is judged whole; migration 012's `bot` key columns keep both states on the fast path; the row-level predicate is `chq.BotFilter`, shared with `shared.activity`'s four event reads and `shared.profiles`' `Get`/`GetByExternalId`/`List`, which carry the same toggle per request; migration 013 keys `distinct_id_activity_states` by `bot` so a crawler stops listing as a derived person, while erasure stays bot-blind) → [`docs/architecture/insights.md`](docs/architecture/insights.md), user flow plan → [`docs/architecture/user-flow.md`](docs/architecture/user-flow.md)
...
- **Profiles** — read API (ClickHouse-backed) serving identified profiles ∪ **derived anonymous persons** (unclaimed `distinct_id`s synthesized from the activity rollup — no rows written; claim exclusion via aliases/profile-ids/external-ids; `Get` redirects claimed anon ids to the canonical profile), activity summary, **server-side session list** (`GetProfileSessions` — one row per `session_id`, keyset-paginated with a sort-bound cursor; the count and the list are separate queries and agree only on an `include_bots` read), property model, soft-delete, device subscriptions → [`docs/architecture/profiles.md`](docs/architecture/profiles.md)
...
- **Billing entitlement** — what an org is *allowed* to send and what it is *priced* at, beside metering's count of what it did. A Go rate-card catalog (not rows — repricing mints a new slug and retires the old, and every org is pinned to the card in force when its mandate activated), one nullable `billing_entitlements` row per org (migrations 019 and 021), negotiated deals as per-org money on that row (`flat_fee_cents` and/or `block_rate_cents`, the allowance in `included_events_override`), an append-only history, the meter's `PUG_USAGE_RESCAN_DAYS` doubling as the invoicing grace (a period is priced that many days after it ends, so raising it delays every invoice), and every entitlement status **derived at read time** from the clock and the ledger, so there is no stored status and no sweep job. **Pricing is one pure function** — `Price(card, events)` / `PriceCustom(terms, events)`, which the dashboard estimate, the invoicing pass, the CLI preview and the tests all call, so a customer can never be shown one number and charged another. Usage rounds to the **nearest** 100,000-event block, ties up, then prices through **graduated** tiers (volume pricing would let a customer pay less by sending more). Read-only over the wire: `GetBillingStatus` and `GetUpcomingInvoice` sit on the viewer floor (`authz.ResourceBilling`), `ListInvoices` is admin-only (`authz.ResourceInvoice`) because a receipt carries a company's billing details, and `included_events`/`retention_days`/`amount_cents` are `Int64Value` wrappers because **absent means no quota / no retention bound / no priced amount and must never render as 0**. **Retention is a plan term nothing enforces**: 5 years on every card and deal (a per-org override applies whenever the row is present and unlapsed) is rendered, never pruned on — no TTL, delete job or query clamp acts on event history, so pug keeps everything. It resolves to **nil, not 1825**, with billing off or on a slug the catalog dropped. Enforcement is a structural invariant *against*: no ingestion path imports the package and no event is ever rejected, throttled or dropped for a quota or an unpaid invoice — a quota drives a banner. `PUG_BILLING_ENABLED` off (the default, and the self-hosted shape) resolves every org with no quota at all. The operator CLI is `pug billing` (`show`/`set`/`extend-trial`/`clear`/`preview`/`invoice void|retry`) — Postgres only, ungated by the switch so a deployment can be prepared before it goes on, and the **only** writer of `billing_entitlements`; an omitted flag keeps the stored value and its empty value clears it, since the common re-`set` is a renewal → [`docs/architecture/usage-billing.md`](docs/architecture/usage-billing.md)
- **Payments** — taking money, beside the entitlement that says what an org may send. A merchant of record behind `billing.PaymentProvider` (**Dodo Payments** is the only implementation, `internal/deps/dodo`), a **mandate-only** self-serve checkout that authorizes a card and charges nothing, a customer portal, a signed Standard-Webhooks inbox at `/billing/webhooks/<provider>` mounted **outside** the Connect chain, a `cmd/cron/billing-reconcile` pass and a `cmd/cron/billing-invoice` pass (both standalone binaries, not `pug cron` subcommands; the invoicer is meant to run hourly, but nothing in the repo sets a cadence — the CronJob schedule does). **Pug owns the quota AND the price; the provider owns the mandate, tax, the receipt and the payout** — an on-demand charge carries an amount, and the only system that can compute it is the one holding the count, so there is no second authority to disagree with and **one Dodo product** (`PUG_DODO_MANDATE_PRODUCT`) every org authorizes against. **No charge without a durable invoice row first**: the row commits `charging` before the API call, so a process that dies between the two leaves a row that says so. Dodo's charge endpoint takes no idempotency key, so **a retry never guesses** — an ambiguous outcome (timeout, 5xx, reset) is settled by *reading* `Payments.List` for the charge's `metadata.invoice_id`, never by charging again. The SDK's own transport retry is disabled on that one call for the same reason, and a decline is an **allow-list of one**, `402` — every other 4xx is pug's problem and stays ambiguous, or a rotated key or a route a dependency bump changed would dun every customer at once. A `404` claiming the mandate is unchargeable is acted on only when a `FetchSubscription` read corroborates it, and a pass that writes off more than ten mandates exits non-zero, since at that scale it is pug's fault and not ten customers'. Dunning is pug's: soft declines retry at +3d/+7d/+7d then go `uncollectible`, hard declines go there at once, a new payment method re-opens them, and `PAST_DUE` is derived from the ledger so it clears the instant a retry succeeds — but nothing ages it out, so an `uncollectible` invoice holds it until a retry, a new card or an operator `void`. **A returning buyer is confirmed by `ConfirmCheckout`, not by the webhook** — it re-reads the checkout from the provider and applies it through the same CAS, because polling pug's own state verifies nothing and makes the checkout moment depend on a reachable webhook URL, which a self-hosted or local deployment does not have; the `session_id` it takes is a claim, so the subscription must carry the `metadata.org_id` pug wrote **and** a `checkout_ref` that resolves to the same org. One writer per row: the operator writes `billing_entitlements`; the invoicing pass and payment webhooks write `billing_invoices`; the subscription webhook, reconcile and confirm write `billing_subscriptions` through the one CAS in `applySubscription`, so no second notion of "newer" exists. A delivery is attributed on a `checkout_ref` pug mints and stores in `billing_checkout_sessions` **before** it opens the checkout (which is also where the card is pinned), never on `metadata.org_id` — static payment links accept `metadata_*` query parameters, so a payload's org id names an org rather than proving one. Every unapplicable delivery is stored, marked processed and NOT retried (foreign currency, no status, no customer, negative price, **not on-demand**, unattributable) — retries fix none of them; a recurring subscription is refused at the boundary because the provider would bill it on its own schedule *and* pug would invoice it. Two deliveries ARE retried: a second *live* subscription for one org, whose ordinary cause is a cutover whose cancellation has not landed; and a body pug cannot **decode**, which a redeploy inside the retry window fixes. The CAS tie-breaks in the withholding direction: a webhook's stamp is the whole-second `webhook-timestamp` header, so an equal stamp can end a subscription but never revive one. `past_due` is **live**: a failed card is worth a banner, never a degraded product. Cancellation is where usage billing loses money, so pug pins Dodo's `next_billing_date` past its own next charge, closes the period early on a scheduled cancellation **that lands before the natural close** (closing early otherwise would spend the period's one invoice slot on a few days and leave the rest unbillable), and offers `RemovePaymentMethod`, which closes-charges-then-cancels in that order and refuses to cancel at all unless the charge settled. Enforcement stays out here too → [`docs/architecture/usage-billing.md`](docs/architecture/usage-billing.md)
...
- **NATS / JetStream** — the deployed cluster, the eight-stream `schema/nats/` inventory, and the `pug nats migrate` PreSync reconciler. Note it is a **reconciler, not a migration**, but only in the create/update direction: it drives every stream and consumer named in the YAML to the YAML's values, so a manual `nats stream update` is silently undone by the next ArgoCD sync — and it **never deletes**, so a stream dropped or renamed in the YAML survives on the cluster, still bound to its old subject filter and still accruing data. Removing one is a manual `nats stream rm`. Stream **replica count is the one stream field not in the YAML** — it is `NATS_STREAM_REPLICAS` (default `1`), because the right value is per-deployment and `schema/nats/streams.yaml` is baked into the image; the job refuses `< 1` (a present-but-empty env var parses to `0`, which the server would silently coerce to 1, de-replicating every stream) and refuses `> 1` against a non-clustered server, since a standalone NATS accepts the scale-up and only loses the streams at its next restart. The DLQ retry **backoff schedule** (`defaultBackOff()`, `internal/deps/nats/worker.go`) is the one policy hardcoded in Go — `discard: new` and `max_deliver: 5` are YAML fields (`schema/nats/streams.yaml`, `schema/nats/consumers.yaml`), changeable without a code edit.

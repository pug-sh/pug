...
- **Tile kinds:** `insight` (persisted `insight_query` JSONB holding a `shared.insights.v1.InsightQuerySpec` + `view_mode` column) or `markdown` (`markdown_body`). The time window and granularity are **dashboard-level**, not per-tile — a tile only stores *what* it measures.
- **Tile customization columns:** `compare` (text proto enum name for `ComparePeriod`, FE renders the comparison via a second `Query` with a shifted window), `thresholds` (jsonb array — typed as `[]byte` via a per-column sqlc override since the global jsonb-as-map mapping can't hold a top-level array, serialized via `protojson`), `header` and `visualization` (jsonb objects round-tripped via `MessageToMap` / `MapToMessage`). All four are evaluated client-side; the server only stores and returns them.
- **Dashboard-level window:** `dashboards.default_time_range` (a `common.v1.TimeRangePreset` enum name) and `default_granularity` (a `shared.insights.v1.Granularity` enum name) are dedicated columns; one time picker drives the whole board. `view_mode` stays per-tile (each chart's visualization). `Create` / `Update` full-replace the dashboard window + display fields; `UpdateTile` full-replaces `view_mode`, `layouts`, and `insight_query` (the spec) — clients must send them on every update or they reset.
- **`Upsert` RPC:** atomic dashboard edit — full-replaces metadata and reconciles the tile set in a single transaction. Tiles with empty `id` are inserted; tiles with a matching `id` are updated; existing tiles whose `id` is omitted from the request are deleted. UPDATE statements are gated in SQL on `payload_hash <> $new_hash`, so byte-identical writes don't fire the `moddatetime` trigger and `update_time` stays meaningful. The dashboard metadata UPDATE is gated symmetrically on `tiles_changed OR metadata changed`. The response returns tiles in **request order** so clients can map server-assigned ids back to a local draft. The contract is last-write-wins — no optimistic locking; concurrent edits aren't detected server-side. CreateTile / UpdateTile / DeleteTile remain valid for other callers and maintain the same `payload_hash` invariant.
- **`payload_hash`:** sha256 over `proto.MarshalOptions{Deterministic: true}` of a `DashboardTileInput` built from the **normalized** tile fields (view_mode + compare go through their normalizers first), with `id` left zero. Computed in `TilePayload.Encode()` so every write path — `CreateDashboardTile`, `UpdateDashboardTile`, `UpsertDashboardTile…` — stores the hash through the same function. Existing rows from before migration 014 have an empty-bytea default, which can never match a sha256, so they always rewrite on first Upsert.

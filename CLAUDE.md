...
> **Migration 017 drops `projects.private_api_key` / `public_api_key` in the same migration that backfills them into `api_keys`.** No dual-write, no `*Legacy` fallback lookups, no marker value standing in for a column that must hold something, no phase 2. The tradeoff is a deploy window: a pod still running the previous image selects those columns and fails until it rolls. Taken deliberately at current traffic (one live project, no signups mid-deploy) — the transitional alternative (dual-write + fallback-read + a second migration to drop) was several hundred lines and its own class of bugs, notably that a legacy read-fallback silently un-revokes a deleted key. `TestApiKeysBackfill` is the load-bearing test: 017's backfill is the single chance every existing key gets to survive, with no fallback behind it and no catch-up pass to fix a miss.

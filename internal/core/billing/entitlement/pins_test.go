...
func TestTheUnpinnedSlugsAgreeBetweenGoAndSQL(t *testing.T) {
	...
	for _, slug := range slugs {
		// custom's check constraint demands a price; no other slug needs one.
		var fee any
		if slug == SlugCustom {
			fee = int64(1)
		}
		...
		if _, err := pg.PgW.Exec(ctx,
			`insert into billing_entitlements (org_id, plan_slug, flat_fee_cents)
			 values ($1, $2, $3)`, orgs[slug], slug, fee); err != nil {

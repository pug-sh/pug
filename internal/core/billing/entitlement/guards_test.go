...
func TestWithOrgLockReadsOnlyOnceItHoldsTheLock(t *testing.T) {
	...
	if _, err := grant.Exec(ctx,
		`insert into billing_entitlements (org_id, plan_slug, included_events_override, flat_fee_cents, provider_product_id)
		 values ($1, 'custom', 5000000, 40000, 'prod_acme')`, f.orgID); err != nil {

...
const (
	...
	// Reserved for the roadmap but NOT activated in this foundation: the proto
	// OrgRole enum and the org_members check constraint still permit only
	// admin/member, so no row can carry these and no policy grants them yet.
	// Activating one means: add it here, grant it below, extend the hierarchy,
	// and add the proto enum value + DB constraint value.
	//   roleOwner  = "ORG_ROLE_OWNER"   // inherits admin; + org:delete, billing:manage
	//   roleViewer = "ORG_ROLE_VIEWER"  // read-only across project-scoped resources
)
...
// groupingRules define the role hierarchy (g): admin inherits member.
// Roadmap: owner -> admin, admin -> viewer when those roles are activated.
var groupingRules = [][]string{
	{roleAdmin, roleMember},
}

// policyRules is the role -> permission matrix. It encodes TODAY's behavior
// exactly (this is a zero-behavior-change foundation):
//
//   - member: full CRUD on every project-scoped resource, plus read-only on the
//     org-level entities a member can already see (the org itself, the member
...
//
// Role assignment lives in Postgres, not here. Adding a permission is a one-line
// edit; it is reviewed like code.
var policyRules = buildPolicyRules()

func buildPolicyRules() [][]string {
	var rules [][]string

	// member — full CRUD on all project-scoped resources (current reality:
	// any org member has full access to all project data).
	...
	// member — read-only on org-level entities (view org, list members/projects).
	rules = append(rules, grant(roleMember, ResourceOrg, ActionRead)...)
	rules = append(rules, grant(roleMember, ResourceMember, ActionRead)...)
	rules = append(rules, grant(roleMember, ResourceProject, ActionRead)...)

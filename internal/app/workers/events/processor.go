...
func (p *Processor) ProcessMessage(ctx context.Context, data []byte) error {
	...
	chBatch, err := p.ch.PrepareBatch(ctx, "INSERT INTO events (bot_score, event_id, project_id, distinct_id, kind, auto_properties, custom_properties, occur_time, session_id)")
	if err != nil {

...
var permissionRegistry = map[string]authzspec.Spec{
	...
	"/dashboard.billing.v1.BillingService/GetUpcomingInvoice":    authzspec.OrgGated(authz.ResourceBilling, authz.ActionRead, "on the viewer floor: it is the usage everyone sees, priced"),
	"/dashboard.billing.v1.BillingService/ListInvoices":          authzspec.OrgGated(authz.ResourceInvoice, authz.ActionRead, "admin-only; a receipt carries the company's billing details"),
	"/dashboard.billing.v1.BillingService/RemovePaymentMethod":   authzspec.OrgGated(authz.ResourceBilling, authz.ActionCreate, "admin-only; it charges the card and cancels the mandate"),

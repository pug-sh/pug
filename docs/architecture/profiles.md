...
- **Cookieless ids never become persons.** Migration 011's `WHERE NOT startsWith(distinct_id, 'cookieless-')` on the activity MV means a cookieless visitor (server-derived daily-rotating id, see [ingestion.md](ingestion.md)) mints no derived anonymous person — without it, every visitor-day would create a ghost. `GetByID` of a cookieless id is `ErrProfileNotFound`; the claimed-id machinery is untouched. Identify/alias with a cookieless id is structurally rejected by `anonymous_id`'s existing `^$|^anon-` pattern (pinned by `TestIdentify_RejectsCookielessAnonymousID`). **Bare-id erasure of a cookieless id works**, via a deliberate exception. `RequestErasureByID` routes through `hasErasableActivity`, which probes `distinct_id_activity_states` for ordinary ids but the `events` table directly for `cookieless-`-prefixed ones — 011 keeps them out of that rollup by design, so the rollup is structurally blind to ids whose events plainly exist. Erasing on a probe that cannot see the data would answer "not found" for data the system still holds, which is the wrong failure direction for an Art. 17 obligation. The original objection (a cookieless id is unknowable to its own visitor, so no data subject can present one) is true of a *visitor*-initiated DSAR but says nothing about a *controller*: `GetEventExplorer` and `GetActivityFeed` both return cookieless ids — they carry no cookieless filter, only a default bot exclusion that `include_bots` lifts, so a *bot-tagged* cookieless id needs that toggle to be visible. Erasure itself never consults `bot`, so erase-by-id works whether or not the id was surfaced. `RequestErasureByExternalID` / `DeleteDataSubject` carry no activity probe and erase as before. Pinned by `TestErasure_ByID_CookielessIsErasable`, which runs a consented control through the identical entry point so a regression in ordinary erasure cannot hide behind it.
...
- Activity fields come from `distinct_id_activity_states` (`schema/clickhouse/migrations/005_create_profile_activity_summary.sql`), an incremental `AggregatingMergeTree` rollup keyed by `(project_id, distinct_id, bot)`.
- The per-IDENTIFIED-profile activity summary is built by `profileActivitySummaryCTE`: it joins the shared `identity_union` CTE (`profiles.IdentityUnionCTE` — the canonical profile ID, external_id, and all alias IDs of every non-deleted profile; also consumed by the identity-resolving insights builders, see [insights.md](insights.md#identity-resolution-person-stitching)) to `distinct_id_activity_states`, then re-aggregates to one row per `(project_id, profile_id)`. Derived anonymous persons carry their own single-`distinct_id` summary from `anonPersonsCTE` — no alias fan-out to re-aggregate.
...
- **Bot-tagged traffic never becomes a person by default.** Migration 013 keys `distinct_id_activity_states` by `bot`, so `Get` / `GetByExternalId` / `List` carry an `include_bots` toggle (default false) that drops bot-only distinct_ids from the person set and leaves tagged events out of every `ProfileActivitySummary` — a row-level `bot = 0`, exact here because the table is keyed per `distinct_id` rather than per session (see [bot-detection.md](bot-detection.md)). An id with both kinds of traffic splits into two rows that merge back when bots are included. Only *derived anonymous* persons drop out of the person set — an identified profile is always listed, with its tagged events left out of the activity summary (`personsCTE` unions unfiltered `latest_profiles` with the filtered `anon_persons`). Erasure is deliberately exempt: `hasErasableActivity` and the compliance deletes name no `bot`, so a tagged id stays discoverable and erasable. Pinned by `TestProfilesBotExclusion` and `TestErasure_ByID_BotTaggedIsErasable`. `ProfileActivitySummary.bot` marks a person whose *every* counted event was tagged (`min(states.bot)` — an identified user with one tagged event among many is not a crawler), so it is only ever true on an `include_bots` read; it needs no migration, since 013's key column is the source. Migration 013 is **not** backfilled: rows the MV wrote between the 012 tagging deploy (v0.0.19) and 013 aggregated without `bot` and read as 0, so a crawler active in that window keeps listing as a person — `AggregatingMergeTree` never rewrites states, so the window is permanent.

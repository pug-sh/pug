...
- **Raw path:** `cookielessExclusionCond` (`internal/core/insights/cookieless.go`) injects `NOT startsWith(distinct_id, 'cookieless-')`; the decision helpers are `excludeCookielessForAgg` (metric-scoped) and `excludeCookielessForPersons` (insight-scoped), plus one unconditional site — `BuildSegmentUsersQuery` passes `true`, since `SegmentUsersRequest` carries no `InsightQuerySpec` and so has no toggle to consult. Threading is per-branch: the multi-event single-scan trends builder puts the predicate **inside each event's `countIf`/`uniqIf` condition**, never the shared WHERE — a mixed TOTAL+UNIQUE_USERS tile must keep counting all traffic in the TOTAL branch. Retention excludes in both its cohort scan and the `e.`-aliased retained scan; top-K-of-USERS excludes regardless of metric (it ranks people).

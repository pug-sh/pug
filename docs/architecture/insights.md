...
- **USER dimension**: events resolve to a canonical user key in SQL via the shared `identity_union` CTE (`profiles.IdentityUnionCTE` — see [Identity Resolution](#identity-resolution-person-stitching)), `LEFT ANY JOIN` so pathological multi-matches can't multiply rows; unidentified distinct_ids stay as themselves. The winning rows are then **enriched** by a second small lookup (`BuildTopKProfilesQuery`, not query-cached) attaching `TopKProfile{id, external_id, properties}`; the `$others` row and unidentified distinct_ids stay un-enriched.
...
- **Profile filters**: `PROPERTY_SOURCE_PROFILE` filters match events by `distinct_id IN (profile ids ∪ external_ids ∪ alias ids)` — the full identity set of every matching profile, so a filtered ranking and the identity union agree on which ids are a person's (they can still disagree on which profiles are live — see `profileFilterCondition`'s note on raw vs latest rows). Pinned by `TestIntegration/top_k/user_profile_filter` and `TestIdentityResolution/profile_filter`.
...
Map (`INSIGHT_TYPE_MAP`) measures a metric per country for a choropleth. Configured by `InsightQuerySpec.map` (a `MapQuery`), which is self-contained like `top_k`/`user_flow`/`session`: `spec.events`, `session`, `breakdowns` and `breakdown_limit` must be unset (CEL `map_no_events` / `map_no_session` / `map_no_breakdowns` / `map_no_breakdown_limit`).

- **The dimension is fixed to `$country`** and is not a field. That is the whole reason this is an insight type rather than a chart style for [Top K](#top-k): `view_mode` is a client hint the server never validates, so `MAP` over a `$browser` top K would be unguarded end to end. Moving the country into the spec is what brings the pairing inside the enforced envelope at all.
- **`MapQuery` is `{scope, metric, metric_property}`** — an optional `common.v1.EventFilter` scope (same reuse as `TopKQuery.scope`), a metric reusing `AggregationType` (UNSPECIFIED → TOTAL; SUM/AVG/MIN/MAX require `metric_property` and nothing else may set it, CEL `map_metric_property_required` / `map_metric_property_only_for_numeric`). Top-level `filter_groups` apply as usual.
- **No user-facing limit, no `$others`.** An overflow bucket has no shape on a map. `mapCountryLimit` (250) is a backstop against a project whose `$country` carries junk — the enricher only overwrites a client-supplied `$country` when the geo provider resolves one, so the dimension is best-effort, not closed. Truncation past 250 is silent.
- **Executed as a top K.** `topKRequestForMap` (`internal/core/insights/map.go`) rewrites a MAP request into the equivalent TOP_K one — `DIMENSION_PROPERTY` on `$country`, `limit: 250`, `omit_others: true`, metric and scope carried through — and `ExecuteQuery` runs it down the existing top-K path. The raw builders, `canUseTopKRollup` and the result assembly are therefore unaware map exists, and a map rides the same **rollup fast path** top K already had (`$country` is a `materializedDims` entry, so an unfiltered map over a day-aligned window is served from `dashboard_event_rollup_daily`). Spec-level state (`filter_groups`, `include_cookieless`) rides across on the clone rather than by named copy.
- **`mapCountryLimit` deliberately exceeds `TopKQuery.limit`'s `lte: 100`.** That is only safe because the rewrite happens inside `ExecuteQuery`, downstream of every `protovalidate` call — the rewritten request must never be re-validated. Pinned by `TestTopKRequestForMap/is_deliberately_not_schema-valid`.
- **Response**: `QueryResponse.top_k` — the same `TopKResult` rows, keyed by ISO alpha-2. `keepISOCountries` enforces that key by testing membership in `geo.IsCountryCode` (the 249 assigned ISO 3166-1 alpha-2 codes plus `XK`, which Cloudflare returns for Kosovo). It drops the empty-country row (both paths group on `coalesce(country, '')`, so unresolved traffic arrives under `""`) **and** any value a client wrote to `$country` itself — the enricher overwrites it only when the provider resolves one, so `"USA"`, `"unknown"` and correctly-shaped-but-unassigned codes like `"ZZ"` all reach the query. A shape check alone would pass that last group. Ranking happens before the filter, so junk codes can still crowd real countries past `mapCountryLimit`; fixing that properly belongs in ingestion (validate the CF header, strip a client-supplied `$country`), not in the query. The rows therefore do **not** sum to the project's total. There is no `MapResult`: it would be `TopKRow` minus `is_others` and `profile`, and a client that already parses `top_k` needs no new result-parsing code.
- **Granularity** is inert for the *result* yet still required and still selects the applicable cap — exactly as for top K; see the granularity note below.
- The accuracy and duplicate-delivery caveats from [Top K](#top-k) apply unchanged.

Dashboard tiles use `DASHBOARD_TILE_VIEW_MODE_MAP`, which is the only view a map tile takes (the Sankey/user-flow arrangement); the FE pins it on switching to the type and releases it on switching away.
...
`DashboardTile.view_mode` (a `dashboard.dashboards.v1.DashboardTileViewMode`: `LINE`, `AREA`, `BAR_GROUPED`, `BAR_STACKED`, `TABLE`, `KPI`, `SANKEY`, `PIE`, `MAP`) stays per-tile — each chart picks its own visualization. Insight tiles normalize `UNSPECIFIED`/unknown to `LINE`; markdown tiles normalize any mode to `UNSPECIFIED`.
...
`InsightsService.GetFilterSchema` populates the filter/breakdown property picker. Event names come from the `event_names` MV; custom-property keys and the *non-promoted* auto-property keys come from the `property_keys` MV (both migration 004), which enumerates `auto_properties`/`custom_properties` **map keys**. Promoted auto-properties are stripped out of the map into dedicated events columns at ingest, so the MV never sees them — `mergePromotedAutoDimensions` injects **every promoted string column's key** (`clickhouse.PromotedStringAutoProperties`: the twenty `materializedDims` — `$country`, `$region`, `$city`, `$os`, `$browser`, `$browserVersion`, `$device`, `$platform`, `$utmSource`, `$utmMedium`, `$utmCampaign`, `$utmTerm`, `$utmContent`, `$osVersion`, `$pathname`, `$hostname`, `$referrerDomain`, `$channel`, `$locale`, `$screenSize` — plus the promoted-but-not-rolled-up `$url`, `$referrer`, `$pageTitle`) into `AutoPropertyKeys`. Counts/last-seen come from the event rollup (`BuildPromotedAutoPropertyKeysQuery` over `dashboard_event_rollup_daily`, counting non-empty values); keys with no rollup dimension surface at count 0, and a frozen pre-promotion `property_keys` entry for an injected key is dropped in favor of the injected one. Every injected key is always listed (count 0 when the rollup has no rows), typed `STRING`, and re-sorted with the map keys by count; the `allowedTypes` filter still applies (they're excluded from numeric/boolean-only requests — typed injection for the bool/numeric promoted columns remains out of scope). The query engine already reads promoted columns for breakdowns, filters, and value typeahead (`AutoPropertyProjectionFor` / `AutoPropertyDistinctValuesFor`); this only closes the discovery gap. A rollup-query failure (table absent or a transient ClickHouse error) degrades to count-0 discovery rather than failing the whole schema: `mergePromotedAutoDimensions` still lists every promoted key, so the picker stays populated. The foundational `event_names`/`property_keys` fetches still hard-fail. The schema is Redis-cached (key `filterschema:<projectID>[:<eventKind>][:types=...]`, 5-min TTL) — **except** a degraded result is *not* cached, so a transient rollup blip can't freeze count-0 promoted dims (and their corrupted count-sort order) for the full TTL. Once a populated production cache exists, a change to the assembled shape should be paired with a cache-key version bump so stale entries don't linger past TTL.
...
## Identity Resolution (person stitching)

A person accumulates events under up to three distinct_ids: `cookieless-…` pre-consent, `anon-…` pre-identify, then their external_id. Events are immutable, so stitching happens at **read time**: person-based insights join the shared `identity_union` CTE (`profiles.IdentityUnionCTE` in `internal/core/profiles/identity.go` — id ∪ external_id ∪ alias ids of every non-deleted profile) onto the events scan and group by the canonical user key `if(i.profile_id = '', e.distinct_id, i.profile_id)` (`profiles.IdentityUserKeyExpr`).

- **Resolving builders**: funnel counts (`buildFunnelWindowFunnel`), funnel timing (both scans — the windowFunnel pre-filter and the tagged CTE), retention (both scans — cohorts and return_events, joined on `user_key`), and top-K USER. The profiles activity summary reuses the same CTE. So a split person converts as one funnel chain, reads as retained, and ranks as one user.
- **Deliberately not resolving**: trends/segmentation `UNIQUE_USERS`/`PER_USER_AVG` still count raw `distinct_id` — the hottest path, where "unique visitors" arguably *should* count pre- and post-signup sessions separately; a decision, not an oversight. User flow groups by session, not person.
- **Profile filters mirror the same set** (`profileFilterCondition`): a `PROPERTY_SOURCE_PROFILE` filter compiles to `distinct_id IN (id ∪ external_id ∪ alias ids of matching profiles)` as a self-contained subquery — it cannot reference the CTE (a `Condition` embeds in arbitrary queries), so the id sources are mirrored, with the doc comment cross-linking both. The mirror covers the id sources only: the filter reads raw `profiles`, the CTE reads `latest_profiles`, so an un-merged superseded row can pass the filter while the CTE drops it. The non-empty `external_id` guard is scoped to that one branch, matching the CTE's per-value `arrayFilter`, so a never-identified profile still resolves by its id and aliases (`TestIdentityResolution/anonymous_profile_filter_resolves`).
- **Cookieless ids never resolve**: no profile row can carry one (`Identify.anonymous_id` is pinned to `^$|^anon-` and `external_id` rejects the prefix), so a cookieless event's key is always its raw id — resolution cannot merge consent-rejecting strangers. Pinned by `TestIdentityResolution/topk_cookieless_never_merges`.
- **Cost**: ClickHouse inlines a CTE per reference, and `identity_union` reads `latest_profiles` and `latest_profile_aliases` once each, so profiles reads equal the reference count: funnel counts 1×, funnel timing 2× (pre-filter + tagged), retention 3× (`cohorts` is referenced by both `cohort_sizes` and `retained`, plus `return_events`), top-K USER 1×. Analytical counts from the inlining model, not EXPLAIN-verified. Accepted at current scale; the escape hatch is a flat worker-maintained `distinct_id → profile_id` table.

`TestIdentityResolution` (integration) seeds one person split across `anon-…`/external_id plus an unsplit control and pins funnel/retention/timing/top-K/profile-filter behavior end-to-end.
...
- at most one breakdown and, if present, on a materialized dimension (`materializedDims` — twenty since migration 009: the original ten plus `$pathname`, `$hostname`, `$referrerDomain`, `$channel`, `$locale`, `$screenSize`, `$utmTerm`, `$utmContent`, `$browserVersion`, `$osVersion`);
- no filter groups and no per-event filters;
- non-empty event kind;
- DAY/WEEK/MONTH granularity (and UTC bucketing — a non-UTC `timezone` falls back to raw, since the rollup is UTC-day-keyed).
...
## Cookieless Exclusion (`include_cookieless`)

`InsightQuerySpec.include_cookieless` (default false) governs ids prefixed `cookieless-` — the server-derived daily-rotating identities of consent-rejecting visitors ([ingestion.md](ingestion.md)). Because one returning human becomes a new id every day, those ids are **excluded by default** from user-counting metrics (UNIQUE_USERS and PER_USER_AVG — numerator and denominator alike, wherever they appear: trends, segmentation, top K) and from person-based insights (funnel, retention, user flow, and USER-dimension top K — rotating ids zero cross-day retention and break sequences). TOTAL, numeric aggregations, and **every session metric** always count all traffic — a consent-rejector's pageview is still a pageview, and session builders never read `distinct_id`, so the flag is inert for session queries by construction.

- **Raw path:** `cookielessExclusionCond` (`internal/core/insights/cookieless.go`) injects `NOT startsWith(distinct_id, 'cookieless-')`; the decision helpers are `excludeCookielessForAgg` (metric-scoped) and `excludeCookielessForPersons` (insight-scoped), plus one unconditional site — `BuildSegmentUsersQuery` passes `true`, since `SegmentUsersRequest` carries no `InsightQuerySpec` and so has no toggle to consult. Threading is per-branch: the multi-event single-scan trends builder puts the predicate **inside each event's `countIf`/`uniqIf` condition**, never the shared WHERE — a mixed TOTAL+UNIQUE_USERS tile must keep counting all traffic in the TOTAL branch. The identity-resolving builders' event scans are `e.`-aliased under the join, so their predicates read `e.distinct_id`: funnel counts, retention in both scans (cohorts + return_events), funnel timing in both scans (pre-filter + tagged), and top-K-of-USERS regardless of metric (it ranks people). User flow is person-based for the toggle but is not identity-joined, so it stays unaliased.
- **Rollup path:** migration 011 added a `cookieless UInt8` **key column** to `dashboard_event_rollup_daily` (computed in the MV as `toUInt8(startsWith(distinct_id, 'cookieless-'))`), so exclusion is `WHERE cookieless = 0` and inclusion is simply no predicate (states merge across both key values). **Both toggle states stay on the fast path**; the eligibility predicates (`canUseEventRollup`, `canUseTopKRollup`) do not consult the flag. Breakdown top-N ranking is per event: `top_grain_<i>` carries that event's predicate (and aggregates at the query's bucket grain), `top_vals_<i>` sums it to pick the top N — so ranking and values cover one population. Because membership is per event, zero-filling absent cells is too: `fillMultiEventTrendZeros` skips a value another event names when this one folded it into `$others`.
- Builder-level pins: `TestCookielessExclusion_RawBuilders` / `TestCookielessExclusion_RollupBuilders`; end-to-end (counts, toggle, rollup↔raw parity, funnel): `TestIntegrationCookieless`.
...
- `AVG_EVENTS_PER_SESSION` — mean count of (scoped) events per session; with a `page_view` scope this is **pages per session**. A first-class metric rather than an FE division of two tiles because pageviews are occur-day-windowed while sessions are keyed on session start and never clipped — dividing tiles disagrees visibly on boundary days. Raw form `avg(event_count)` over the per-session CTE; rollup-eligible via the existing `event_count_state`.
- `ENTRY` / `EXIT` — count of sessions bucketed by their **first** / **last** matching event's breakdown value (entry/exit page). These require TRENDS with **exactly one** breakdown (CEL `session_page_metrics_require_trends_breakdown`); the other metrics accept either insight type and any breakdown count.
...
**Rollup fast path.** `canUseSessionRollup` (`session_rollup.go`) routes eligible session queries to the `dashboard_session_rollup` rollup (migration 007, extended by 010), under the same `rollupWindowAligned` day-alignment guard (and UTC-bucketing gate) as the event rollup. Eligibility: trends or segmentation; DAY/WEEK/MONTH granularity; no filter groups, no per-event scope filters; at most one breakdown on a materialized session dimension (`sessionMaterializedDims` — sixteen since migration 010: the original eleven plus `$pathname`, `$referrerDomain`, `$channel`, `$utmTerm`, `$utmContent`); and for ENTRY/EXIT, trends with exactly one breakdown. Anything rejected falls back to the raw builders with identical results. **Accuracy caveat:** `BOUNCE_RATE` and `AVG_EVENTS_PER_SESSION` are affected by duplicate deliveries (a redelivered event inflates the rollup's `countState`-based `event_count`, so bounce under-reports and events-per-session over-reports, while the raw path self-corrects); SESSIONS/ENTRY/EXIT (session-id groups) and AVG_DURATION (idempotent min/max) are immune. Pinned by `TestIntegration/session_rollup_bounce_duplicate_overcount_documented`. → [clickhouse.md](clickhouse.md)

**First-touch acquisition panels are session-grain.** Only a session's landing event carries the external referrer; later pageviews self-refer (blanked to `''`) and usually carry no UTM, so event-grain `$channel` degrades toward `Direct` mid-session. Referrers/Channels panels should therefore query `SESSIONS` with an entry-state breakdown (`$referrerDomain` / `$channel`) — first-touch attribution from the session rollup — and web stat-row session metrics should scope `kind=page_view` (which also keeps entry states from being claimed by URL-less events). The event-grain `$channel` dimension still exists in the event rollup for discoverability and kind-scoped exploration, with this caveat attached.

...
- **Response**: `QueryResponse.top_k` — the same `TopKResult` rows, keyed by ISO alpha-2. Both the raw and rollup paths group on `coalesce(country, '')`, so traffic whose country never resolved arrives as an empty-key row; `dropUnresolvedCountry` removes it, since a choropleth cannot draw it and leaving it in skews any client that normalizes over the returned rows. The rows therefore do **not** sum to the project's total. There is no `MapResult`: it would be `TopKRow` minus `is_others` and `profile`, and a client that already parses `top_k` needs no new result-parsing code.

...
- **Profile filters**: `PROPERTY_SOURCE_PROFILE` filters match events by `distinct_id IN (profile ids ∪ external_ids ∪ alias ids)` — the full identity set of every matching profile, so a filtered ranking and the identity union agree on which events are a person's. Pinned by `TestIntegration/top_k/user_profile_filter` and `TestIdentityResolution/profile_filter`.
...
- **Profile filters mirror the same set** (`profileFilterCondition`): a `PROPERTY_SOURCE_PROFILE` filter compiles to `distinct_id IN (id ∪ external_id ∪ alias ids of matching profiles)` as a self-contained subquery — it cannot reference the CTE (a `Condition` embeds in arbitrary queries), so the definition is mirrored, with the doc comment cross-linking both. The non-empty `external_id` guard is scoped to that one branch, matching the CTE's per-value `arrayFilter`, so a never-identified profile still resolves by its id and aliases (`TestIdentityResolution/anonymous_profile_filter_resolves`).
- **Cookieless ids never resolve**: no profile row can carry one (identify() is disabled in cookieless mode; the proto rejects the prefix), so a cookieless event's key is always its raw id — resolution cannot merge consent-rejecting strangers. Pinned by `TestIdentityResolution/topk_cookieless_never_merges`.
- **Cost**: ClickHouse inlines a CTE per reference, and `identity_union` reads `latest_profiles` twice (the ARRAY JOIN branch and the alias branch's own join), so count in profiles reads to compare against the top K figures above: funnel counts 2×, funnel timing 4×, retention 6× (`cohorts` is referenced by both `cohort_sizes` and `retained`, plus `return_events`). Analytical counts from the inlining model, not EXPLAIN-verified. Accepted at current scale; the escape hatch is a flat worker-maintained `distinct_id → profile_id` table.
...
- **Raw path:** `cookielessExclusionCond` (`internal/core/insights/cookieless.go`) injects `NOT startsWith(distinct_id, 'cookieless-')`; the decision helpers are `excludeCookielessForAgg` (metric-scoped) and `excludeCookielessForPersons` (insight-scoped), plus one unconditional site — `BuildSegmentUsersQuery` passes `true`, since `SegmentUsersRequest` carries no `InsightQuerySpec` and so has no toggle to consult. Threading is per-branch: the multi-event single-scan trends builder puts the predicate **inside each event's `countIf`/`uniqIf` condition**, never the shared WHERE — a mixed TOTAL+UNIQUE_USERS tile must keep counting all traffic in the TOTAL branch. The person-based builders' event scans are all `e.`-aliased under the identity join, so their predicates read `e.distinct_id`: retention excludes in both scans (cohorts + return_events), funnel timing in both scans (pre-filter + tagged), and top-K-of-USERS regardless of metric (it ranks people).

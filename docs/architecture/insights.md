...
- **Raw path:** `cookielessExclusionCond` (`internal/core/insights/cookieless.go`) injects `NOT startsWith(distinct_id, 'cookieless-')`; the decision helpers are `excludeCookielessForAgg` (metric-scoped) and `excludeCookielessForPersons` (insight-scoped). Threading is per-branch: the multi-event single-scan trends builder puts the predicate **inside each event's `countIf`/`uniqIf` condition**, never the shared WHERE — a mixed TOTAL+UNIQUE_USERS tile must keep counting all traffic in the TOTAL branch. Retention excludes in both its cohort scan and the `e.`-aliased retained scan; top-K-of-USERS excludes regardless of metric (it ranks people).
- **Rollup path:** migration 011 added a `cookieless UInt8` **key column** to `dashboard_event_rollup_daily` (computed in the MV as `toUInt8(startsWith(distinct_id, 'cookieless-'))`), so exclusion is `WHERE cookieless = 0` and inclusion is simply no predicate (states merge across both key values). **Both toggle states stay on the fast path**; the eligibility predicates (`canUseEventRollup`, `canUseTopKRollup`) do not consult the flag. Breakdown top-N ranking (`top_vals`) applies the same predicate as the query's metric so ranking and values cover one population.
